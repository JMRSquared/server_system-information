  ...
  export default {
    ...
    data() {
      return {
        ...
        URL: 'https://jmrsquared.com/api/stats',
        routes: ['time', 'system', 'osInfo', 'cpu', 'bios', 'baseboard', 'services', 'currentload', 'processes', 'mem']
      };
    },
    mounted() {
      this.GetStats();
      ...
      GetStats() {
        setInterval(() => {
          this.routes.forEach(route => {
            axios.get(`${this.URL}/${route}`).then(data => {
              this.sendStats(route.toUpperCase(), data);
            })
          });
        }, 10000);
      },
